...
def create_table(conn):
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS processed_jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, job_link TEXT, post_date TEXT, job_response TEXT, title_text TEXT)")


def save_processed_job(conn, job_link, post_date, job_response, title_text):
    cursor = conn.cursor()
    cursor.execute("INSERT INTO processed_jobs (job_link, post_date, job_response, title_text) VALUES (?, ?, ?, ?)", (job_link, post_date, job_response, title_text))
    conn.commit()
...
def load_processed_jobs():
    ...
    cursor = conn.cursor()
    sql = "SELECT job_link FROM processed_jobs"
    sql_exec = cursor.execute(sql).fetchall()
  
    processed_jobs = []
    for row in sql_exec:
        processed_jobs.append(row[0])
    # for row in sql_exec:
    #     job = {
    #         "id": row[0],
    #         "job_link": row[1],
    ...
    try:
        ...
        for page in range(1, 10):
            ...
            for item in job_items:
                ...
                job_link = item.find("div", class_="title").find("a")["href"].strip()
                ...
                if job_link is not None:
                    ...
                    hash_tag = [i.text.strip() for i in item.find(class_="col-sm-8 text-muted dot_divided text_field d-sm-flex").find(class_="flex-wrap")] if item.find(class_="col-sm-8 text-muted dot_divided text_field d-sm-flex").find(class_="flex-wrap") else ""
                    favorite_task = ["парсер","парсинг","спарсить", "python", "пайтон", "питон", "flask", "django", "fastapi", "php", "js", "пхп", "javascript", "джанго", "фласк","фастапи"]

                    if any(word in title_text or word in description_element or word in hash_tag for word in favorite_task):
                        job_link_save = f"https://www.weblancer.net{job_link}"
                        post_date, title_time = extract_post_date(item)
                        if job_link_save not in processed_jobs:
                            save_processed_job(conn, job_link_save, title_time, job_response, title_text)    
            time.sleep(1)
        time.sleep(60)
